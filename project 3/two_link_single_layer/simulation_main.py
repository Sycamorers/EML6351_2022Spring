...
if __name__ == '__main__':
    ...
    L = 20
    Lmod = 4*L+1
    dyn = dynamics.Dynamics(alpha=alpha,betar=betar,betaeps=betaeps,gammath=gammath,gammaw=gammaw,lambdaCL=lambdaCL,YYminDiff=YYminDiff,kCL=kCL,tauN=tauNoise,phiN=phiNoise,phiDN=phiDNoise,phiDDN=phiDDNoise,L=L,deltaT=deltaT,useCL=useCL,useNN=useNN,useYth=useYth)
    ...
    for jj in range(0,len(t)):
        ...
        rNormHist[jj] = np.linalg.norm(rmj)
    ...
    # plot the error norm
    eNplot, eNax = plot.subplots()
    eNax.plot(t, rNormHist, color='darkviolet', linewidth=0.5, linestyle='-')
    eNax.set_xlabel("$t$ $(sec)$")
    eNax.set_ylabel("$\Vert r \Vert$")
    eNax.set_title("Error Norm RMS = " + str(np.around(np.sqrt(np.mean(rNormHist ** 2)), decimals=2)))
    eNax.grid()
    eNplot.savefig(path + "/filterederrorNorm.pdf")
    ...
    for ii in range(5):
        thHax.plot(t,thetaHHist[ii,:],color=np.random.rand(3),linewidth=0.5,linestyle='--')
    ...
    fax.plot(t,fHist[0,:],color='darkviolet',linewidth=0.5,linestyle='-')
    fax.plot(t,fHHist[0,:],color='darkviolet',linewidth=0.5,linestyle='--')
    fax.plot(t,fHist[1,:],color='limegreen',linewidth=0.5,linestyle='-')
    fax.plot(t,fHHist[1,:],color='limegreen',linewidth=0.5,linestyle='--')
    ...
    fdax.plot(t,fDiffNormHist,color='darkviolet',linewidth=0.5,linestyle='-')
    ...
    #plot the approx norm
    fdplot,fdax = plot.subplots()
    fdax.plot(t[TCLindex:],fDiffNormHist[TCLindex:],color='darkviolet',linewidth=0.5,linestyle='-')
    fdax.set_xlabel("$t$ $(sec)$")
    fdax.set_ylabel("$function$")
    ...
    eigax.plot(t,lambdaCLMinHist,color='darkviolet',linewidth=0.5,linestyle='-')
    ...
    for ii in range(L):
        sigmaax.plot(phidsigmas[0,:],sigmaHist[4*ii,:],color=np.random.rand(3),linewidth=0.5,linestyle='-')
    ...
    sigmaplot,sigmaax = plot.subplots()
    for ii in range(L):
        sigmaax.plot(phidsigmas[1,:],sigmaHist[4*ii+1,:],color=np.random.rand(3),linewidth=0.5,linestyle='-')
    ...
    sigmaplot,sigmaax = plot.subplots()
    for ii in range(L):
        sigmaax.plot(phiDdsigmas[0,:],sigmaHist[4*ii+2,:],color=np.random.rand(3),linewidth=0.5,linestyle='-')
    ...
    sigmaplot,sigmaax = plot.subplots()
    for ii in range(L):
        sigmaax.plot(phiDdsigmas[1,:],sigmaHist[4*ii+3,:],color=np.random.rand(3),linewidth=0.5,linestyle='-')
