...
if __name__ == '__main__':
    ...
    phiax.plot(t,phidHist[0,:],color='darkviolet',linewidth=0.5,linestyle='-')
    phiax.plot(t,phiHist[0,:],color='darkviolet',linewidth=0.5,linestyle='--')
    phiax.plot(t,phidHist[1,:],color='limegreen',linewidth=0.5,linestyle='-')
    ...
    eNax.set_title("Filtered Error Norm RMS = "+str(np.around(np.sqrt(np.mean(rNormHist**2)),decimals=2)))
    eNax.grid()
    ...
    for ii in range(2*L):
        WHax.plot(t,WHHist[ii,:],color=np.random.rand(3),linewidth=0.5,linestyle='--')
    ...
    for ii in range(5*l):
        VHax.plot(t,VHHist[ii,:],color=np.random.rand(3),linewidth=0.5,linestyle='--')
    ...
    eigax.plot(t,lambdaCLMinHist,color='darkviolet',linewidth=2,linestyle='-')
    eigax.plot([TCL,TCL],[0.0,lambdaCLMinHist[TCLindex]],color='black',linewidth=0.5,linestyle='-')
