...
if __name__ == '__main__':
    ...
    dyn           = dynamics.Dynamics(alpha=alpha,betar=betar,betadel=betadel,gammath=gammath,gammaw=gammaw,tauN=tauNoise,phiN=phiNoise,phiDN=phiDNoise,phiDDN=phiDDNoise,l=l,
                                      useNN=useNN,useYth=useYth,useBias=useBias,useCuda=useCuda,memorySize=memorySize,batchSize=batchSize,numberEpochs=numberEpochs,learnRate=learnRate)
    ...
    for ii in range(5):
        thHax.plot(t,thetaHHist[ii,:],color=rand(3).numpy(),linewidth=0.5,linestyle='--')
    ...
    for ii in range(2*L):
        WHax.plot(t,WHHist[ii,:],color=rand(3).numpy(),linewidth=0.5,linestyle='--')
    ...
    lsax.plot(t,lossHist,color='darkviolet',linewidth=0.5,linestyle='-')
